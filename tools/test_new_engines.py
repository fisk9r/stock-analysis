...
def main():
    ...
    # 2026-09-05 #492：买点候选再升级——买区/卖区**区间明确**（用户要求）。
    # 断言校验：决策+动作 / 买区区间 / 挂单价 / 卖区区间 / 空间% / 止损 / 分 / 理由。
    check("收盘推送·买点候选含决策(现在买)", "✅ **现在买**" in _cl["text"])
    check("收盘推送·买点候选含买入区间与挂单价",
          "买区9.50~10.20" in _cl["text"] and "挂单10.00" in _cl["text"])
    check("收盘推送·买点候选含卖出区间", "卖区11.00~12.00" in _cl["text"])
    check("收盘推送·买点候选含空间%", "**+20.0%**" in _cl["text"])
    check("收盘推送·买点候选含止损", "止损8.50" in _cl["text"])
    check("收盘推送·买点候选含综合分", "71分" in _cl["text"])
    check("收盘推送·买点候选含一句话理由", "加速上行趋势 · 半导体" in _cl["text"])
