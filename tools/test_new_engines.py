...
def main():
    ...
    # 2026-09-05 #487：买点候选改为「一句话决策」格式（旧格式一行塞 8 个字段
    # 却没有结论，用户反馈「看半天不知道买不买」）。断言同步改为校验决策要素：
    # 决策符号+动作 / 买价 / 目标价 / 空间% / 止损 / 综合分 / 一句话理由。
    check("收盘推送·买点候选含决策(现在买)", "✅ **现在买**" in _cl["text"])
    check("收盘推送·买点候选含买价与目标",
          "买10.00" in _cl["text"] and "→12.00" in _cl["text"])
