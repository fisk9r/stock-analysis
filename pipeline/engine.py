...
def auction_discipline(streak):
    ...
    2026-08-29 st=2 归因修正（rec_picks 44 条实测）：st=2 的 2-5% 弱高开子桶
    胜率仅 14.3% / -2.35%（全样本同桶 +2.26% 显著反向），而 gap≥5% 子桶
    66.7% / +5.84% 正常——st=2 胜率异常全是「弱高开接二板」造成的。
    因此 st=2 的高开门槛从 2% 收紧到 5%（二板接力只认强确认）。
    """
    if (streak or 0) >= 3:
        return {
    ...
    if (streak or 0) == 2:
        return {
            "rule": "高开≥5%才跟进（2板接力只认强确认，2-5%弱高开历史胜率仅14%）/ 低开≤-2%放弃 / 其余观望",
            "basis": "st=2实测44条：gap≥5%胜率66.7%/+5.84%，但gap 2-5%仅14.3%/-2.35%（全样本同桶+2.26%反向）——二板弱高开是派发陷阱"}
    return {
